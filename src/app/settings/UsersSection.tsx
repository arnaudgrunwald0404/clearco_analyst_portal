...
export default function UsersSection() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      console.log('📊 [UsersSection] Response status:', response.status)
      console.log('📊 [UsersSection] Response headers:', Object.fromEntries(response.headers.entries()))

      let result
      try {
        result = await response.json()
        console.log('📊 [UsersSection] Response data:', result)
      } catch (jsonError) {
        console.error('❌ [UsersSection] Failed to parse JSON:', jsonError)
        const textResponse = await response.text()
        console.error('❌ [UsersSection] Raw response:', textResponse)
        throw new Error(`Invalid JSON response: ${textResponse.substring(0, 200)}`)
      }

      if (!response.ok || !result.success) {
        const errorMessage = result.error || `HTTP ${response.status}: ${response.statusText}`
        console.error('❌ [UsersSection] API Error:', errorMessage)
        throw new Error(errorMessage)
      }

      console.log('✅ [UsersSection] Successfully fetched users:', result.data?.length || 0)
          ...
          <div className="flex items-center">
            ...
            {isSuperAdmin ? (
              <span className="inline-flex items-center gap-1">
                ...
                Super Admin - All Users Across All Domains
              </span>
            ) : (
              <>
          ...
          <div className="overflow-x-auto">
            <table className="w-full divide-y divide-gray-200">
              <thead className="bg-pink-200 rounded-t-lg">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-900 text-bold uppercase tracking-wider w-1/2">
                    User
                  </th>
                  <th className="px-3 py-3 text-left text-xs font-medium text-gray-900 text-bold uppercase tracking-wider w-32">
                    Dates
                  </th>
                  <th className="px-3 py-3 text-left text-xs font-medium text-gray-900 text-bold uppercase tracking-wider w-24">
                    Actions
                  </th>
                </tr>
              </thead>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="mt-2 flex items-center gap-2 flex-wrap">
                          ...
                          {isSuperAdmin && user.domain && (
                            <span className="inline-flex items-center gap-1 px-2 py-1 text-xs font-medium rounded-full bg-purple-100 text-purple-800" title="User's domain">
                          ...
                          <>
                            {showDeleteConfirm === user.id ? (
                              ...
                              <button
                                ...
                                className="inline-flex items-center px-3 py-1 text-xs font-medium rounded text-red-700 hover:bg-red-200 transition-colors disabled:opacity-50"
