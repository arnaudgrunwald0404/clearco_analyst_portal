...
export default async function OurBriefings({ analystId: analystIdProp }: { analystId?: string }) {
  ...
  try {
    const settingsResp = await fetch(`${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/settings/general`, { cache: 'no-store' })
    if (settingsResp.ok) {
      try {
  ...
  const resp = await fetch(`${baseUrl.replace(/\/$/, '')}/api/briefings?${params.toString()}`, {
    cache: 'no-store',
    headers,
  })
  let briefings: any[] = []
